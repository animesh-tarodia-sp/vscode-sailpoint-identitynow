...
import { BaseTreeItem, IdentityAttributeTreeItem, isIdentityAttributeTreeItem, isSourceTreeItem, isTransformTreeItem, SourceTreeItem, TransformTreeItem } from "../models/ISCTreeItem";
...
export class OpenDependencyPanelCommand {
    ...
    async execute(node: BaseTreeItem): Promise<void> {
        if (isTransformTreeItem(node)) {
            ...
            DependencyPanel.createOrShow(
                ...
                node.label as string,
                node.label as string);
        } else if (node.contextValue === "cloud-rule") {
            DependencyPanel.createOrShow(
                this.extensionUri,
                node.tenantId,
                node.tenantName,
